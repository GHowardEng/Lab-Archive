...
bool CControlPi::open(string com)
{
	return true;
}
...
CControlPi::CControlPi()
{
	...
	gpioSetMode(STAT, PI_INPUT);
	gpioSetMode(PASS_BUTTON, PI_INPUT);
	gpioSetMode(REJ, PI_INPUT);
	
	//gpioSetMode(LED, PI_OUTPUT);
	gpioSetMode(SM0, PI_OUTPUT);
	
	gpioSetMode(STAT_LED, PI_OUTPUT);
	gpioSetMode(R_LED, PI_OUTPUT);
	gpioSetMode(P_LED, PI_OUTPUT);
