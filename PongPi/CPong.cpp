...
void CPong::update()
{
	...
	paddle_center = Point2f(paddle_center.x, paddle_center.y + (paddle_vel * _time));
	...
	if (position.x - RADIUS < paddle_center.x + PADWIDTH / 2 && position.y - RADIUS < (paddle_center.y + PADHEIGHT/2) && position.y + RADIUS > (paddle_center.y - PADHEIGHT/2))
	{
		...
		cout << "Ball: " << velocity.y << " Pad: " << paddle_vel << "\n"; 
		velocity.y += 0.3*paddle_vel;
		
		if (velocity.y > VMAX - VMAX/3)
		{
			velocity.y = VMAX - VMAX/3;
		}
		else if (velocity.y < -(VMAX - VMAX/3))
		{
			velocity.y = -(VMAX - VMAX/3);
		}
...
void CPong::draw()
{
	...
	{
		stringstream stream;
		stream << fixed << setprecision(2) << ((1 / frame_time));
	...
	putText(_canvas, score, Point(100, 30), FONT_HERSHEY_SIMPLEX, 0.5, Scalar::all(255));
	putText(_canvas, frames, Point(XSIZE - 240, 30), FONT_HERSHEY_SIMPLEX, 0.5, Scalar::all(255));
	...
	// Reset end time
	end_time = std::chrono::steady_clock::now() + std::chrono::milliseconds(33);
